...
const SliderCreate = (props) => {
  ...
  const moveBack = () => {
    if (previous) {
      ...
      if (
        el === slidersRef.current.children[0] ||
        el === slidersRef.current.children[1]
      )
        return false;
      else {
...
const SliderCreateJoinRedux = connect(mapStateToProps)(SliderCreate);

export default SliderCreateJoinRedux;
